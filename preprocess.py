...
def rename_song():
    #改名，将原来歌曲命名更改为vctk格式的
    p = 'exp_part2_1/data/VCTK-Corpus/wav16'
    files = librosa.util.find_files(p, ext='wav')
    
    for one_path in files:
        head, tail = os.path.split(one_path)

        if tail.startswith('p'): continue #already processed

        singer = os.path.basename(head)
        newname = os.path.join(head, f'{singer}_{tail}') 
        os.rename(one_path, newname)
        print(head, tail)
