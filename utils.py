...
def world_encode_wav(wav_file, fs, frame_period=5.0, coded_dim=128):
...
def coded_sp_stat(coded_sp):
    m = np.mean(coded_sp, axis=0, keepdims=True)
    s = np.std(coded_sp, axis=0, keepdims=True)
    return m, s
