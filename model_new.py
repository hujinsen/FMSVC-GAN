...
class Generator(nn.Module):
    ...
    def __init__(self, input_dim, params):
        ...
        embedder_net.load_state_dict(torch.load('/home/hu/disk1/EXP_Part2/PyTorch_Speaker_Verification/speech_id_checkpoint/final_epoch_950.model'))
        embedder_net.eval()
        # style encoder
    ...
    def get_style_encoder(self):
        return self.enc_style

    def forward(self, images):
        # reconstruct an image
        content, style_fake = self.encode(images)
        # print(f'--content:{content.shape}--style--{style_fake.shape}--')
        images_recon = self.decode(content, style_fake)
        return images_recon

    def convert(self, src, trg):
        src_content = self.enc_content(src) #
        trg_speaker = self.enc_style(trg) #
      
        adain_params = self.mlp(trg_speaker)
        self.assign_adain_params(adain_params, self.dec)
        new_content = self.dec(src_content)
...
class StyleEncoder(nn.Module):
    ...
    def forward(self, x):
        return self.model(x) #torch.Size([batchsize, 16, 1, 1])

class SpeechEmbedder(nn.Module):
    
    def __init__(self):
        super(SpeechEmbedder, self).__init__()    
        self.LSTM_stack = nn.LSTM(128, 768, num_layers=3, batch_first=True)
    ...
    def forward(self, x):
        ...
        x = torch.transpose(x, 1, 2) #交换帧数和特征维度
        x, _ = self.LSTM_stack(x.float()) #(batch, frames, n_mels)
        #only use last frame
        x = x[:,x.size(1)-1] #(batch, n_mels)
        x = self.projection(x.float())#(batch, 256)
...
if __name__ == "__main__":
    ...
    g = Generator(1, config['gen']).get_style_encoder(t)
    print(g.shape)

    # d = MsImageDis(1, config['dis'])(g)
    # print(len(d))
    # print(d[0].shape)

    t = torch.randn((1, 500, 128))
    s = SpeechEmbedder()(t)
    print(s.shape)